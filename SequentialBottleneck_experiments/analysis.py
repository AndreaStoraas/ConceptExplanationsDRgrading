...
def binary_accuracy(output, target):
    ...
    acc = (pred.int()).eq(target.int()).sum() #Get number of correctly predicted observations
    acc = acc*100 / np.prod(np.array(target.size())) #Divide by number of observations
    return acc
