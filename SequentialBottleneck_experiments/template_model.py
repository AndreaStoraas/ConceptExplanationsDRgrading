...
def inception_v3(pretrained, freeze, **kwargs):
    ...
    if pretrained:
        ...
        #Added by Andrea (weights from the InceptionV3 finetuned on our training dataset)
        print('Loading best Inception V3 weights from DR classification...')
        chkpoint_path = '../../output/CroppedCLAHE_InceptionV3_EarlyStopping.pt'
        model.load_partial_state_dict(torch.load(chkpoint_path, map_location='cpu'))
        #Try pretrained on imagenet instead:
        #print('Loading the model weights from Imagenet...')
...
class Inception3(nn.Module):
    ...
    def __init__(self, num_classes, aux_logits=True, transform_input=False, n_attributes=0, bottleneck=False, expand_dim=0, three_class=False, connect_CY=False):
        ...
        n_attributes: number of attributes (=concepts) to predict
