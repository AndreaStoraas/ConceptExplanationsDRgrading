...
#Following the Concept Bottleneck Models paper by Koh et al. (2018), but changed neural network architecture to Densenet-121
#Device:
...
output_path = "../output"
#What we save the model as after training:
...
#Order of the six concepts in the df are: MA, HE, SoftEx, HardEx, NV, IRMA
...
#NB! If batch-size for the validation/test set increases above 1, the attribution loss calculation must be modified!
valid_loader = DataLoader(valid_dataset, batch_size=1, shuffle=False, num_workers=4)

########### New code specifically for running Densenet (not Inception V3) --------------------
#Customized forward function for Densenet-121:
def forward(self,x):
...
#Load in the weights for the best performing model on the DR datasets for TCAV (with 5 DR classes):
n_classes = 5 
