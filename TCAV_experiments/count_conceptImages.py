...
#Counting how many images where the different concepts are present
...
'''
#Start with FGDAR:
fgadr_conceptOverview = pd.read_csv('BottleneckModels/FGADR_training/FGADR_Concept_DR_annotations.csv', index_col = 'Unnamed: 0')
...
#Next, inspecting DDR:
ddr_conceptOverview = pd.read_csv('BottleneckModels/DDR_conceptOverview.csv', index_col='Unnamed: 0')
...
for i in range(ddr_conceptOverview.shape[0]):
    ...
    if (ddr_conceptOverview.iloc[i,1]==0) and (ddr_conceptOverview.iloc[i,2]==0) and (ddr_conceptOverview.iloc[i,3]==0) and (ddr_conceptOverview.iloc[i,4]==0):
        normal_counter += 1
        #print('No MA')
        #print(ddr_conceptOverview.iloc[i,1])
...
#DiaretDB
diaretDB1_overview = pd.read_csv('BottleneckModels/DiaretDB1_conceptOverview.csv',index_col = 'Unnamed: 0')
...
'''
#IDRiD:
IDRiD_conceptOverview = pd.read_csv('BottleneckModels/IDRiD_conceptOverview.csv',index_col='Unnamed: 0')
